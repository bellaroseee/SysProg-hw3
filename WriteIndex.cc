...
static int WriteHeader(FILE *f,
  ...
  while (i < (doctableBytes + memidxBytes)) {
...
static int WriteHashTable(FILE *f,
  ...
  for (int i = 0; i < ht->num_buckets; i++) {
    ...
    int32_t numElements = LinkedList_NumElements(bucket);
    ...
    // BUCKET CONTENT
    // only write if chain is not empty
    if (numElements != 0) {
...
static int WriteHTBucketRecord(FILE *f,
  ...
  BucketRecord bucketRec(numElts, bucketOffset);
  bucketRec.toDiskFormat();
  ...
  if (fwrite(&bucketRec, sizeof(BucketRecord), 1, f) != 1) {
    return kFailedWrite;
  }
...
static int WriteHTBucket(FILE *f,
  ...
  for (int i = 0; i < numElts; i++) {
    ...
    ElementPositionRecord elPosRec(elementPos);
    elPosRec.toDiskFormat();
    if (fwrite(&elPosRec, sizeof(ElementPositionRecord), 1, f) != 1) {
      LLIterator_Free(it);
      return kFailedWrite;
    }
